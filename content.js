...
(function () {
  if (window.__resumeATSLoaded) return;
  window.__resumeATSLoaded = true;
  ...
  // Returns true so Chrome keeps the channel open while storage.get is async.
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.action === 'openPanel' || msg.action === 'autoFillOnly') {
      chrome.storage.local.get(['apiKey', 'resumeExtractedText', 'latexSource'], data => {
        S.apiKey = data.apiKey || '';
        S.resumeText = data.resumeExtractedText || '';
        S.latexSource = data.latexSource || '';
        S.jobDescription = extractJobDesc();
  ...
  // ── Job description ────────────────────────────────────────────────────
  function extractJobDesc() {
    const title = document.title ? document.title + '\n\n' : '';
    const sel = [
      '[data-testid="job-description"]', '.job-description', '.jobDescriptionContent',
      '.description__text', '#job-description', '[class*="jobDescription"]',
      '.show-more-less-html__markup', '.jobs-description__content',
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      if (cached) {
        ...
        const prompt = `You are an ATS expert. Analyze this resume vs the job description.
...
Required JSON:
{
  ...
  "suggestions": [
    {
      ...
      "title": "<short title>",
      "description": "<why this helps ATS>",
      "impact": "+N points",
      "section": "Skills|Summary|Experience|Education",
      "originalText": "<exact verbatim LaTeX text to find and replace>",
      "replacementText": "<the replacement LaTeX text>"
    }
  ],
  "profileData": {
  ...
  "companyName": "<company name extracted from job description>"
}

CRITICAL RULES FOR SUGGESTIONS:
1. originalText MUST be an EXACT character-for-character copy from the LaTeX source above. Even whitespace and LaTeX commands must match perfectly.
2. NEVER add new lines or new \\item entries. Instead, REPLACE an existing similar skill or bullet point with a modified version that incorporates the missing keyword.
3. For Technical Skills: find an existing skill that is LEAST relevant to this job and replace it with the missing keyword. For example, replace "Redis" with "Kubernetes" — do NOT add a new line.
4. For Experience bullets: reword an existing \\item to naturally include the missing keyword.
5. replacementText must be valid LaTeX and maintain the same structure (same number of lines, same commands).
6. Provide 5-8 suggestions. Every suggestion MUST have a non-empty originalText that exists verbatim in the resume.`;
  ...
  function submitApp() {
    const btns = [...document.querySelectorAll('button,input[type=submit]')]
      .filter(b => !b.closest('#resumeats-panel') &&
        /submit|apply/i.test((b.textContent || '') + b.type + (b.getAttribute('aria-label') || '')));
    if (btns[0]) {
      btns[0].scrollIntoView({ behavior: 'smooth', block: 'center' });
      btns[0].style.outline = '3px solid #6c63ff';
      toast('✓ Submit button highlighted!');
      setTimeout(() => { if (btns[0]) btns[0].style.outline = ''; }, 5000);
    } else {
      toast('Submit button not found — submit manually', true);
    }
  }
